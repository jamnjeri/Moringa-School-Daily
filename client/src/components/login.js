...
const Login = ({ handleLogin }) => {
    ...
    .then((response) => {
      ...
      if (response.ok) {
        response.json().then((user) => {
          ...
          } else if (user.role === "technical writer") {
            navigate("/newsfeed");
          }
        });
      } else {
  ...
  return (
    <section className="gradient-form h-full bg-white dark:bg-white flex items-center justify-center h-screen">
